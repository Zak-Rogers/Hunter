...
public class RabbitAgent : Agent
{
    ...
    [SerializeField] Material carrotEaten;
    ...
    void OnTriggerEnter(Collider other)
    {
        ...
        if (other.tag=="Rabbit")
        {
            if(energy >= 80)
            {
                ...
                wait = true; 
                mating = true; 
                spawnSystem.SpawnObject("Rabbit");
            }
            else
            {
                other.transform.localPosition = new Vector3(Random.Range(maxSpawnArea, minSpawnArea), 1, Random.Range(maxSpawnArea, minSpawnArea));
    ...
    void FixedUpdate()
    {
        ...
        if(wait)
        {
            if(eating)
            {
                ...
                {
                    particlesTriggered = TriggerParticles(eatingParticles[0]);
                    particlesTriggered = TriggerParticles(eatingParticles[1]);
                }
            }
            if (mating)
            {
